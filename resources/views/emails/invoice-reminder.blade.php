...
<head>
    ...
    <title>Rappel de paiement - {{ $invoice->invoice_number }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .header-badge { display: inline-block; padding: 4px 14px; border-radius: 999px; font-size: 11px; font-weight: 700; margin-top: 12px; }
        .header-badge.overdue  { background: #ef4444; color: #fff; }
        .header-badge.upcoming { background: #f59e0b; color: #fff; }
        .header-badge.reminder { background: #2563eb; color: #fff; }
        ...
        .alert-overdue { background: #fef2f2; border-left: 4px solid #ef4444; color: #991b1b; }
        .alert-info    { background: #eff6ff; border-left: 4px solid #2563eb; color: #1e40af; }
        .alert-ref     { background: #fef2f2; border: 1px solid #fca5a5; color: #991b1b; border-radius: 8px; padding: 12px 16px; text-align: center; font-size: 13px; margin: 16px 0; }
...
<div class="card">
    ...
    <div class="header">
        ...
        <div class="doc-type">Rappel de paiement</div>
        <div class="doc-num">Facture {{ $invoice->invoice_number }}</div>
        @if($type === 'overdue')
            <div><span class="header-badge overdue">Paiement en retard</span></div>
        @elseif($type === 'upcoming')
            <div><span class="header-badge upcoming">Échéance prochaine</span></div>
        @else
            <div><span class="header-badge reminder">Rappel</span></div>
        @endif
    </div>
    ...
    <div class="body">
        ...
        <div class="alert alert-overdue">
            <strong>Votre facture est en retard de paiement</strong>
            La facture <strong>{{ $invoice->invoice_number }}</strong> n'a pas été réglée à la date d'échéance du <strong>{{ $invoice->due_date->format('d/m/Y') }}</strong>.
        </div>
        @elseif($type === 'upcoming')
        <div class="alert alert-warning">
            <strong>Votre facture arrive à échéance</strong>
            La facture <strong>{{ $invoice->invoice_number }}</strong> arrivera à échéance le <strong>{{ $invoice->due_date->format('d/m/Y') }}</strong>.
        </div>
        @else
        <div class="alert alert-warning">
            <strong>Rappel de paiement</strong>
            La facture <strong>{{ $invoice->invoice_number }}</strong> est en attente de règlement.
        </div>
        @endif
