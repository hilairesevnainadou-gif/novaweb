{{-- ════════════════════════════════════════════════════════════════
     resources/views/admin/billing/invoices.blade.php — VERSION CORRIGÉE
     Corrections :
       1. issue_date / due_date → accès sécurisé avec optional() + Carbon
       2. Boutons manquants ajoutés : PDF, Rappel, Modifier, Supprimer
       3. Permissions correctes sur chaque bouton/action
       4. Colonne "Reste" ajoutée dans le tableau
       5. Modal suppression ajouté
       6. Bouton "Paiements" vers la liste des paiements
     ════════════════════════════════════════════════════════════════ --}}
...
<style>
    ...
    .invoices-table { width: 100%; border-collapse: collapse; min-width: 900px; }
    ...
    .action-btn { background: none; border: none; color: var(--text-tertiary); cursor: pointer; padding: 0.35rem; border-radius: 0.375rem; transition: all var(--transition-fast); font-size: 0.95rem; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; }
    ...
    .action-btn.action-warn:hover   { color: #f59e0b; background: rgba(245,158,11,.08); }
...
<div class="filters-container">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        {{-- Bouton mise à jour statuts en retard (admin only) --}}
        @can('billing.invoices.edit')
        <div>
            <button onclick="updateOverdue()" class="btn-reset" title="Mettre à jour les factures en retard">
                <i class="fas fa-sync-alt"></i> MAJ Retards
            </button>
        </div>
        @endcan
    </div>
        ...
        <tbody id="invoicesTableBody">
            ...
            <tr class="table-row"
                ...
                <td>
                    ...
                    @if($isOverdue)
                        <br><span style="color:#ef4444;font-size:0.7rem;">(Retard)</span>
                    @endif
                </td>
                <td>{{ number_format($invoice->total, 0, ',', ' ') }} FCFA</td>
                <td>{{ number_format($invoice->paid_amount, 0, ',', ' ') }} FCFA</td>
                <td>
                    <span class="amount-remaining {{ $isOverdue ? 'overdue' : '' }}">
                ...
                {{--  BOUTONS D'ACTIONS AVEC PERMISSIONS ──────────────── --}}
                <td class="actions-cell">
                    <div style="display:flex;justify-content:flex-end;gap:0.25rem;flex-wrap:nowrap;">
                        ...
                        <a href="{{ route('admin.billing.invoices.pdf', $invoice) }}"
                           class="action-btn" title="Télécharger PDF" target="_blank">
                            <i class="fas fa-file-pdf"></i>
                        </a>
                        @endcan

                        {{--  NOUVEAU — Envoyer par email (sauf payée) --}}
                        ...
                        {{--  NOUVEAU — Rappel de paiement (factures non payées) --}}
                        @can('billing.invoices.send')
                        @if(in_array($invoice->status, ['sent', 'partially_paid', 'overdue']) && ($invoice->client->email ?? false))
                        <button type="button"
                                class="action-btn action-warn reminder-btn"
                                data-id="{{ $invoice->id }}"
                                data-number="{{ $invoice->invoice_number }}"
                        ...
                        <a href="{{ route('admin.billing.invoices.edit', $invoice) }}"
                           class="action-btn" title="Modifier la facture">
                            <i class="fas fa-edit"></i>
                        </a>
                        @endif
                        @endcan

                        {{--  NOUVEAU — Supprimer (seulement brouillons) --}}
                        @can('billing.invoices.delete')
                        @if($invoice->status === 'draft')
                        <button type="button"
                                class="action-btn action-danger delete-btn"
                                data-id="{{ $invoice->id }}"
                                data-number="{{ $invoice->invoice_number }}"
...
{{-- ══════════════════════════════════════════════════════════════════
     MODAL GÉNÉRIQUE (utilisé pour Envoyer, Rappel, Supprimer)
══════════════════════════════════════════════════════════════════ --}}
...
{{-- Formulaire DELETE invisible (CSRF) --}}
<form id="deleteForm" method="POST" style="display:none;">
    @csrf
    @method('DELETE')
</form>

{{-- Formulaire POST rappel --}}
...
document.querySelectorAll('.send-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        openModal(
            'Envoyer la facture',
            `Envoyer la facture "${number}" par email au client ?`,
            'Le client recevra la facture en PDF en pièce jointe.',
            'Envoyer', 'btn-confirm-primary',
            () => {
                ...
                .then(r => r.json())
                .then(data => {
                    if (data.success) { showNotification('Facture envoyée avec succès'); setTimeout(()=>location.reload(),1500); }
                    else { showNotification(data.message || 'Erreur lors de l\'envoi', 'error'); modalConfirmBtn.disabled=false; modalConfirmBtn.textContent='Envoyer'; }
                })
                .catch(() => { showNotification('Erreur technique', 'error'); modalConfirmBtn.disabled=false; modalConfirmBtn.textContent='Envoyer'; });
            }
        );
    });
});
...
document.querySelectorAll('.reminder-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        openModal(
            'Envoyer un rappel',
            `Envoyer un rappel de paiement pour la facture "${number}" ?`,
            'Le client recevra un email de rappel.',
            'Envoyer le rappel', 'btn-confirm-warning',
            () => {
...
document.querySelectorAll('.delete-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        openModal(
            ...
            `Êtes-vous sûr de vouloir supprimer la facture "${number}" ?`,
            'Cette action est irréversible. La facture sera définitivement supprimée.',
            'Supprimer', 'btn-confirm-danger',
            () => {
...
function updateOverdue() {
    openModal(
        ...
        'Passer toutes les factures échues en statut "En retard" ?',
        '',
        'Mettre à jour', 'btn-confirm-warning',
        () => {
