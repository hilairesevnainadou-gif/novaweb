...
@section('title', 'Modifier le projet - NovaTech Admin')
        ...
        <form method="POST" action="{{ route('admin.portfolio.update', $portfolio) }}" enctype="multipart/form-data" id="portfolioForm">
            ...
            <div id="step1" class="step-content active">
                <div class="row">
                    ...
                    <div class="col-4">
                        <div class="form-group-camille">
                            ...
                            <select id="category"
                                ...
                                <option value="site-vitrine" {{ old('category', $portfolio->category) == 'site-vitrine' ? 'selected' : '' }}>Site Vitrine</option>
                                <option value="e-commerce" {{ old('category', $portfolio->category) == 'e-commerce' ? 'selected' : '' }}>E-commerce</option>
                                <option value="application-web" {{ old('category', $portfolio->category) == 'application-web' ? 'selected' : '' }}>Application Web</option>
                                <option value="maintenance" {{ old('category', $portfolio->category) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                <option value="optimisation" {{ old('category', $portfolio->category) == 'optimisation' ? 'selected' : '' }}>Optimisation SEO</option>
                                <option value="autre" {{ old('category', $portfolio->category) == 'autre' ? 'selected' : '' }}>Autre</option>
                            </select>
                            @error('category')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-4">
                        <div class="form-group-camille">
                            ...
                            <input type="date"
                                   ...
                                   value="{{ old('date', $portfolio->date ? $portfolio->date->format('Y-m-d') : date('Y-m-d')) }}">
                            @error('date')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-4">
                        <div class="form-group-camille">
                            ...
                            <select id="project_type"
                                ...
                                <option value="external" {{ old('project_type', $portfolio->project_type) == 'external' ? 'selected' : '' }}>Externe (client)</option>
                                <option value="internal" {{ old('project_type', $portfolio->project_type) == 'internal' ? 'selected' : '' }}>Interne (NovaTech)</option>
                            </select>
                            @error('project_type')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-4">
                        <div class="form-group-camille">
                            ...
                            <input type="text"
                                   ...
                                   value="{{ old('duration', $portfolio->duration ?? '2-3 semaines') }}"
                    ...
                    <!-- NOUVEAU CHAMP: Taille de l'équipe -->
                    <div class="col-4">
                        <div class="form-group-camille">
                            <label class="form-label-camille">
                            ...
                            <input type="text"
                                   ...
                                   value="{{ old('team_size', $portfolio->team_size ?? '2-3 personnes') }}"
                    ...
                    <div class="col-12">
                        <div class="form-group-camille">
                            ...
                            <textarea id="description"
                                      ...
                                      rows="3"
                                      placeholder="Breve description du projet...">{{ old('description', $portfolio->description) }}</textarea>
                            @error('description')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-4">
                        <div class="form-group-camille">
                            ...
                            <textarea id="work_done"
                                      ...
                                      rows="4"
                                      placeholder="Travaux effectues...">{{ old('work_done', $portfolio->work_done) }}</textarea>
                            @error('work_done')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group-camille">
                            ...
                            @if($portfolio->image)
                                ...
                                <div class="current-image">
                                    <img src="{{ asset('storage/' . $imagePath) }}" alt="Image actuelle">
                            ...
                            <div class="form-help">Format: JPG, PNG, GIF. Max: 2MB. Laissez vide pour conserver l'image actuelle.</div>
                            @error('image')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group-camille">
                            ...
                            @if($portfolio->images && count($portfolio->images) > 0)
                                <div class="current-gallery mb-3">
                                    <label class="form-label-camille small">Galerie actuelle :</label>
                                    @foreach($portfolio->images as $image)
                                        ...
                                        <div class="current-gallery-item">
                                            <img src="{{ asset('storage/' . $galleryPath) }}" alt="Image galerie">
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                            ...
                            <div class="form-help">Ajoutez de nouvelles images. Les images existantes seront conservees.</div>
                            @error('images.*')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-12">
                        <div class="form-group-camille">
                            ...
                            <textarea id="content"
                                      ...
                                      rows="15">{{ old('content', $portfolio->content) }}</textarea>
                            @error('content')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group-camille">
                            ...
                            <div class="alert-camille alert-camille-info" style="margin-top: 0.25rem;">
                                ...
                                <div>
                                    ...
                                    <div class="small mt-1">Verifiez que toutes les informations sont correctes avant de sauvegarder.</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    ClassicEditor
        .create(document.querySelector('#content'), {
            ...
            language: 'fr',
            placeholder: 'Redigez le contenu detaille de votre projet...'
        })
        .then(editor => {
    ...
    function updateSteps() {
        ...
        // Rafraichir CKEditor quand on revient a l'etape 2
        if (currentStep === 2 && editorInstance) {
            setTimeout(() => {
    ...
    // Click sur les etapes pour naviguer
    stepItems.forEach((step, index) => {
    ...
    // Charger les technologies existantes depuis le portfolio
    const existingTechnologies = @json($portfolio->technologies ?? []);
    if (Array.isArray(existingTechnologies) && existingTechnologies.length > 0) {
    ...
    if (titleInput && slugInput) {
        titleInput.addEventListener('blur', function() {
            if (!slugInput.value || slugInput.value === '{{ $portfolio->slug }}') {
    ...
    // APERÇU NOUVELLES IMAGES DE GALERIE
    ...
    document.getElementById('portfolioForm').addEventListener('submit', function(e) {
        // Mettre a jour la valeur des technologies
        technologiesHidden.value = JSON.stringify(technologies);

        const submitButton = document.getElementById('submitBtn');
        submitButton.disabled = true;
        submitButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mise a jour en cours...';
    });
</script>
@endpush
